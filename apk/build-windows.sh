#!/usr/bin/env bash
# Build cross100-v{version}.apk on Windows (Git Bash).
# Port of build.sh (which targets Linux/Termux): Windows build-tools ship
# d8/apksigner as .bat wrappers, so we invoke their jars with java directly.
#
# Prerequisites:
...
# ── Derive version name from latest git tag (e.g. v2.0.55) ──
